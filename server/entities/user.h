...
struct User {
    ...
    // 默认构造函数
    User() : userId(0), userType(FactoryUser) {}

    // 带参数的构造函数
    User(int userId, UserType type, const QString& acc, const QString& pwd, const QString& cont, const QString& skill)
        : userId(userId), userType(type), account(acc), password(pwd), contact(cont), expertSkill(skill) {}
