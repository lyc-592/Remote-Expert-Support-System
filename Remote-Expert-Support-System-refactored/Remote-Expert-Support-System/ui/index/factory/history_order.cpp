...
HistoryOrder::HistoryOrder(DatabaseManager* dbManager, QWidget *parent)
...
void HistoryOrder::setupUI()
{
    // 创建主布局 - 使用水平布局，类似于AgendaPage
    ...
    // 信号槽连接
    connect(btnView, &QPushButton::clicked, this, &HistoryOrder::viewDetails);
    connect(btnViewRecord, &QPushButton::clicked, this, &HistoryOrder::viewRecord);
    connect(btnRefresh, &QPushButton::clicked, this, &HistoryOrder::refreshTable);
    connect(tableView, &QTableView::doubleClicked, this, &HistoryOrder::viewDetails);
    connect(btnSearch, &QPushButton::clicked, this, &HistoryOrder::searchOrders);
    connect(searchEdit, &QLineEdit::returnPressed, this, &HistoryOrder::searchOrders);
}

void HistoryOrder::refreshData()
{
    refreshTable();
}

void HistoryOrder::refreshTable()
{
    ...
    QString queryStr = "SELECT "
                       ...
                       "user.user_account as expert_info "    // 用户2的联系信息（专家）
                       "FROM history_order ho "
                       "LEFT JOIN workorder wo ON ho.device_id = wo.device_id "  // 关联工单表获取故障信息
                       "LEFT JOIN device d ON ho.device_id = d.device_id "       // 关联设备表获取设备名称
                       "LEFT JOIN user ON ho.user2_id = user.user_id "          // 关联用户表获取专家信息
    ...
    model->setHeaderData(6, Qt::Horizontal, tr("专家"));

    // 隐藏ID列
    // tableView->hideColumn(0);
}

void HistoryOrder::searchOrders()
{
    if (!dbManager->isConnected()) {
        QMessageBox::warning(this, "数据库错误", "数据库连接已断开");
        return;
...
void HistoryOrder::viewDetails()
{
    ...
    // 创建详情对话框
    OrderDetailDialog *detailDialog = new OrderDetailDialog(id, dbManager, this);
    detailDialog->exec();
    delete detailDialog;
}

void HistoryOrder::viewRecord()
{
    if (!dbManager->isConnected()) {
        QMessageBox::warning(this, "数据库错误", "数据库连接已断开");
        return;
    ...
    // 创建现场记录对话框
    RecordDialog *recordDialog = new RecordDialog(id, dbManager, this);
    recordDialog->exec();
    delete recordDialog;
}
