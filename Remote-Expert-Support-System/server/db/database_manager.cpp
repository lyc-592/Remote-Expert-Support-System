...
QVector<Device> DatabaseManager::getDevice(const int userId) {
    ...
    QSqlQuery query(db);
    query.prepare("SELECT device_id, device_name, device_type FROM device"
...
bool DatabaseManager::addDevice(const QString &name, const QString &type, int userId) {
    if (!isConnected()) return false;

    QSqlQuery query(db);
    query.prepare(R"(
        INSERT INTO device (user_id, device_name, device_type)
        VALUES (:id, :name, :type)
    )");
