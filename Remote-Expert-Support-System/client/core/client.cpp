...
void Client::addAgenda(QVector<int> experts, int device_id,
                       const QString &order_create_time,
                       const QString &predict_time, const QString &fault) {
    clientNetworkManager->addAgendaRequest(this->user->userId, experts, device_id, order_create_time, predict_time, fault);
}
