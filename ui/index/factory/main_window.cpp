    ...
    : client(client), QMainWindow(parent)
{
    ...
    connect(userPage, &UserPage::logoutRequested, this, [this]{
        LoginWindow *login = new LoginWindow();// 重新 new 一个登录窗口
        this->close();                       // 关闭主窗口
        login->show();
    });
    ...
    connect(this->devicePage, &DevicePage::addDeviceSignal, this, [this](QString name, QString type, QString status) { // 1. Lambda 表达式现在接收信号的参数
        this->client->addDevice(name, type, status); // 将接收到的参数传递给 addDevice 方法
    });
    connect(this->devicePage, &DevicePage::removeDeviceSignal, this, [this](QString name) { // 1. Lambda 表达式现在接收信号的参数
        this->client->removeDevice(name);
    });
    ...
    connect(this->client, &Client::removeDeviceResult, this, &MainWindow::onRemoveDeviceReceived);

    connect(client, &Client::addSimulateDataResult,
            this, &MainWindow::onAddSimulateDataReceived);

    connect(client, &Client::nonContactUsersReceived, this, &MainWindow::onNonContactUsersReceived);
    connect(client, &Client::addContactResult, this, &MainWindow::onAddContactResponseReceived);

    connect(this->client, &Client::updateDeviceResult, this, &MainWindow::onUpdateDeviceReceived);
    connect(devicePage, &DevicePage::updateDeviceSignal,
            this, [this](const QString& deviceName, const QString& type, const QString& status) {
                this->client->updateDevice(deviceName, type, status);
            });

    connect(client, &Client::newChatMessage, this, &MainWindow::onNewChatMessage);
}
...
void MainWindow::setupContactsSidebar()
{
    ...
    contactsSidebar->setFixedWidth(300); // 固定宽度
    ...
    // 连接添加联系人按钮点击事件
    connect(addContactBtn, &QPushButton::clicked, this, &MainWindow::onAddContactButtonClicked);
...
void MainWindow::onContactClicked(QListWidgetItem *item)
{
    ...
    connect(chatDialog, &ChatDialog::sendMessageSignal, this, &MainWindow::onSendChatMessage);
...
void MainWindow::onSendChatMessage(int receiverId, const QString& message) {
...
void MainWindow::onRemoveDeviceReceived(bool success, const QString& message) {
    if(success) {
...
void MainWindow::onAddSimulateDataReceived(bool success, const QString& message) {
    if (success) {
        qDebug() << "✅ 自动模拟数据成功:" << message;
    } else {
        qWarning() << "❌ 自动模拟数据失败:" << message;
    }
}
...
void MainWindow::onNonContactUsersReceived(const QList<User> &users)
{
    if (users.isEmpty()) {
...
void MainWindow::onAddContactResponseReceived(bool success, const QString &message)
{
    if (success) {
...
void MainWindow::onNewChatMessage(int contactId, int senderId)
{
    // 更新当前聊天窗口显示
...
void MainWindow::onUpdateDeviceReceived(bool success, const QString& message) {
    if(success) {
        this->client->getDevice(); // 刷新设备列表
        QMessageBox::information(this, "成功", "更新设备: " + message);
    } else {
        QMessageBox::warning(this, "错误", "更新设备: " + message);
    }
}
// ============================
// 析构函数
