...
void MainWindow::initAudio()
{
    ...
    connect(audioInputDevice, &QIODevice::readyRead,
            this, &MainWindow::on_audioDataReady);

    // 连接UDP接收信号
    connect(udpAudioSocket, &QUdpSocket::readyRead,
            this, &MainWindow::on_audioDataReceived);
...
void MainWindow::stopAudio()
{
    // 首先断开所有音频相关的信号
    if (audioInputDevice) {
        disconnect(audioInputDevice, &QIODevice::readyRead, this, nullptr);
    }

    // 停止并删除音频线程
    if (audioSenderThread) {
        qDebug() << "Stopping audio sender thread...";
        audioSenderThread->stop(); // 先设置停止标志
        QThread::msleep(50);      // 给线程一点时间退出
        delete audioSenderThread;
        audioSenderThread = nullptr;
    }

    if (audioReceiverThread) {
        qDebug() << "Stopping audio receiver thread...";
    ...
    // 取消UDP绑定，避免重启时端口冲突
    if (udpAudioSocket->state() == QAbstractSocket::BoundState) {
        udpAudioSocket->close();
    }
...
void MainWindow::init_CameraDevice(const QCameraInfo& device)
{
...
void MainWindow::init_CameraDeviceInfo()
{
...
void MainWindow::handle_Connect()
{
    ...
    // +++ 添加音频线程创建 +++
    // 获取本机音频端口（本地TCP端口+1）
    quint16 localAudioPort = tcpSocket->localPort() + 1;
    remoteAudioPort = portEdit->text().toUShort() + 1; // 远程音频端口

    // 绑定本地音频端口
    if (!udpAudioSocket->bind(QHostAddress::Any, localAudioPort)) {
        qWarning() << "Audio UDP bind failed:" << udpAudioSocket->errorString();
    } else {
        qDebug() << "Audio UDP bound to local port:" << localAudioPort;
    }
...
void MainWindow::on_imageCaptured(int id, const QImage &image)
{
    Q_UNUSED(id);
...
void MainWindow::init_TcpConnection()
{
    ...
    // +++ 添加音频线程创建 +++
    // 获取客户端音频端口（TCP端口+1）
    QHostAddress clientAddress = tcpSocket->peerAddress();
    quint16 clientAudioPort = tcpSocket->peerPort() + 1;
...
void MainWindow::on_toggleCameraBtn_clicked()
{
    if (camera_Obj) {
...
void MainWindow::on_toggleMicBtn_clicked()
{
...
void MainWindow::on_audioDataReady()
{
...
void MainWindow::on_audioDataReceived()
{
    // 数据由接收线程处理，这里不需要做额外处理
}

void MainWindow::on_recordScreenBtn_clicked()
{
    if (!isRecording) {
        // 开始录制
...
void MainWindow::startScreenRecording()
{
    ...
    // 获取程序窗口的几何信息（使用geometry()可能更接近实际内容区）
    ...
    qDebug() << "原始窗口尺寸 (frameGeometry): " << this->frameGeometry().size();
    qDebug() << "原始窗口尺寸 (geometry): " << windowRect.size();
    ...
    videoFileName = documentsPath + "/window_recording_" + timestamp + ".mp4";
    ...
    arguments << "-f" << "x11grab"
              ...
              << "-r" << "30.0"             // 帧率 30 fps
              << "-profile:v" << "high444"  // 使用 high444 配置文件 (注意兼容性与资源消耗)
              << "-level" << "5.1"          // 配置文件级别 5.1
              << "-pix_fmt" << "yuv420p"    // 像素格式 YUV420p (high444 对应 yuv444p, 但你指定了yuv420p，这里会强制转换)
              // 如果要真正使用 high444，这里应该改为 "-pix_fmt yuv444p"
              // 但 yuv420p 兼容性更好，所以先保留
    ...
    if (ffmpegProcess->waitForStarted(3000)) {
        isRecording = true;
        qDebug() << "开始窗口录制，保存到:" << videoFileName;
        ...
        recordScreenBtn->setText("开始录制屏幕");
        recordingStatusLabel->setText("录制状态: 已停止");
        recordingStatusLabel->setStyleSheet("color: gray;"); // 修复了一个小错误：录制状态标签名
    }
}
